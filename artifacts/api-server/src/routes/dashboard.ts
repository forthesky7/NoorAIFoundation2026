...
router.get("/dashboard/summary", authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    return res.json({
      ...
      recentVideos: recentVideos.map(v => ({ ...v, checkpointCount: 0, thumbnailUrl: v.thumbnailUrl || `https://img.youtube.com/vi/${v.youtubeId}/hqdefault.jpg`, createdAt: v.createdAt.toISOString() })),
...
router.get("/dashboard/admin", authMiddleware, adminMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    return res.json({
      ...
      recentUsers: recentUsers.map(u => ({ id: u.id, name: u.name, email: u.email, role: u.role, subscribed: u.subscribed, createdAt: u.createdAt.toISOString() })),
    });
  } catch {
    return res.status(500).json({ error: "Server error" });
  }
